...
def reset_workflow():
    ...
    for key in ["step", "analysis_result", "questions", "answers", "diagnosis", 
                "safety_info", "instructions", "shopping_list", "uploaded_image", "workflow_started"]:
...
def render_progress_bar():
    """Render progress bar."""
    step_labels = ["Загрузка", "Анализ", "Вопросы", "Безопасность", "Инструкция", "Покупки"]
    n = len(step_labels)
    max_step_index = n - 1
...
def main():
    ...
    st.markdown('<div class="subtitle">AI-помощник по ремонту бытовой техники и дома</div>', unsafe_allow_html=True)

    check_api_key()

    # Кнопка управления
    col_left, col_restart, col_right = st.columns([1, 3, 1], vertical_alignment="center")
    with col_restart:
...
def show_analysis_step():
    ...
    st.markdown('<div class="card-title">Результаты анализа</div>', unsafe_allow_html=True)
    ...
    with col3:
        ...
        danger_text = "Высокий" if danger_level == "high" else "Средний" if danger_level == "medium" else "Низкий"
...
def show_questions_step():
    ...
    st.markdown('<div class="card-title">Уточняющие вопросы</div>', unsafe_allow_html=True)
    ...
    if st.session_state.questions:
        with st.form("diagnostic_questions"):
            ...
            with col1:
                submit = st.form_submit_button("Отправить ответы", type="primary", use_container_width=True)
            with col2:
            ...
            if submit and answers:
                ...
                with st.spinner("Анализирую ответы..."):
                    diagnosis = agent.analyze_answers(st.session_state.analysis_result, st.session_state.answers)
                    st.session_state.diagnosis = diagnosis
                st.session_state.step = 3
                st.rerun()
            elif skip:
                with st.spinner("Формирую диагноз..."):
                    diagnosis = agent.analyze_answers(st.session_state.analysis_result, {})
                    st.session_state.diagnosis = diagnosis
                st.session_state.step = 3
                st.rerun()
...
def show_safety_step():
    """Step 3: Safety check."""
    st.markdown('<div class="card-title">Проверка безопасности</div>', unsafe_allow_html=True)
    ...
    if safety_info.get("is_dangerous"):
        st.markdown(f'<div class="danger-box">{safety_message}</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="success-box">{safety_message}</div>', unsafe_allow_html=True)
    ...
    if safety_info.get("requires_professional"):
        ...
        st.write(f"Причина: {safety_info.get('professional_reason', 'Высокая сложность')}")
...
def show_instructions_step():
    ...
    st.markdown('<div class="card-title">Инструкция по ремонту</div>', unsafe_allow_html=True)
...
def show_shopping_step():
    ...
    if not st.session_state.shopping_list:
        with st.spinner("Составляю список..."):
            ...
            st.session_state.shopping_list = {"list": shopping_list, "cost": cost_estimate}

    shopping_data = st.session_state.shopping_list
    formatted = ShoppingAgent.format_shopping_list(shopping_data["list"], shopping_data["cost"])
    st.markdown(formatted)
